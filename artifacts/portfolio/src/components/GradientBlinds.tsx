...
void main(){ vec4 color; mainImage(color,vUv*iResolution.xy); gl_FragColor=color; }`;
    ...
    let program: Program, geometry: Triangle, mesh: Mesh;
    try {
    ...
    const resize = () => {
      ...
      const effective = blindMinWidth > 0
        ? Math.max(1, Math.min(blindCount, Math.floor(rect.width / blindMinWidth)))
        : Math.max(1, blindCount);
    ...
    const loop = (t: number) => {
      ...
      if (!paused && meshRef.current) {
        try { renderer.render({ scene: meshRef.current }); } catch { /* ignore */ }
      }
    };
    rafRef.current = requestAnimationFrame(loop);
  ...
  if (webglFailed) {
    return (
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse 80% 60% at 30% 40%, rgba(192,193,255,0.12) 0%, transparent 60%), radial-gradient(ellipse 60% 50% at 70% 60%, rgba(76,215,246,0.08) 0%, transparent 60%), #0a0a0f',
